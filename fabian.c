...
{ //Declarar un Puntero a imagen
  ...
  printf("\n\n");
	...
	/*for(i=0;i<Img1->alto; i++)
    	for(j=i;j<Img1->ancho; j++)
    		Img1->imx[i*Img1->ancho+j]=0;*/
    
    //Diagonal Izquierda
	/*for(i=Img1->alto-1; i>=0; i--)
    	for(j=i;j>=0; j--)
    		Img1->imx[i*Img1->ancho+j]=0;*/
    
    //Diagonal Derecha
	/*for(i=Img1->alto-1; i>=0; i--)
    	for(j=(Img1->alto-1)-i ;j<Img1->ancho; j++)
    		Img1->imx[i*Img1->ancho+j]=0;*/
    
    /*
	//Barras Verticales
    int ancho_barra = Img1->ancho/7;
    int espacio = ancho_barra;
    int cont=0;
	...
	while(cont<3)
	{
		...
		for(i=0; i<Img1->alto; i++)
    		for(j=ancho_barra ;j<ancho_barra+espacio; j++)
    			Img1->imx[i*Img1->ancho+j]=0;
    	
    	ancho_barra += (espacio * 2);
    	cont++;
    }*/
    
    /*
	//Barras Horizontales
    int espacio = Img1->ancho/6;
    int inicio = 0;
    int cont=0;
	...
	while(cont<3)
	{
		...
		for(i=inicio; i <inicio+espacio; i++)
     ...
     /*//Ajedrez
    int espacio = Img1->ancho/5;
    int inicio_x = 0, inicio_y = 0;
    int cont=0,aux=0;
	while(cont<9)
	{
		for(i=inicio_x; i<inicio_x+espacio; i++)
    		for(j=inicio_y;j<inicio_y+espacio; j++)
    			Img1->imx[i*Img1->ancho+j]=0;
	...
	int sur = Img1->ancho-1;
    int fin_sur = 1;
    
    int oeste = Img1->alto-1;
    int fin_oeste = 1;
    
    int norte = 1;
    int fin_norte = (Img1->alto-1)-(espacio-1);
    
    int este = 1;
    int fin_este = (Img1->ancho-1)-(espacio-1);
    
    int lineas = Img1->ancho-1;
	
	while(lineas>0)
	{
		// Sur
		for(i=sur+1; i>fin_sur; i--)
    		Img1->imx[i*Img1->ancho+sur]=0;

    		sur-=espacio;
    		fin_sur+=espacio;
			
		//Oeste
		for(i=oeste; i>fin_oeste; i--)
    		Img1->imx[fin_oeste*Img1->ancho+i]=0;
    		
    		oeste-=espacio;
    		fin_oeste+=espacio;
    		
    	//Norte
		for(i=norte; i<=fin_norte; i++)
    		Img1->imx[i*Img1->ancho+norte]=0;
    		
    		norte+=espacio;
    		fin_norte-=espacio;
    	
    	//este
		for(i=este; i<fin_este; i++)
    		Img1->imx[fin_este*Img1->ancho+i]=0;
    ...
    gcPutImgBmp("Resultado7.bmp",Img1);
