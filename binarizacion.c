...
//gcIMG* Binarizar(gcIMG *img, unsigned int umbral);

int main(void)
{ //Declarar un Puntero a imagen
    gcIMG *Imagen1,*Imagen2;
    Imagen1=gcGetImgBmp("dos.bmp");
    Imagen2=Binarizar(Imagen1);

    /*
    Imagen2=gcNewImg(Imagen1->ancho,Imagen1->alto);//Generando nueva imagen con las mismas dimensiones de la imagen original
    for(k=0;k<Imagen1->size;k++)
    {
        if(Imagen1->imx[k]<128)
            Imagen2->imx[k]=0;//Binarizacion
        else
            Imagen2->imx[k]=255;//Binarizacion
    }*/
    
    gcPutImgBmp("Binarizacion2.bmp",Imagen2);
