...
{ //Declarar un Puntero a imagen
    ...
    unsigned int k,umbral1,umbral2;
    ...
    {
        if(Imagen1->imx[k]<umbral1)
            Imagen2->imx[k]=0;//Binarizacion
        if((Imagen1->imx[k]>=umbral1)&&(Imagen1->imx[k]<umbral2))
            Imagen2->imx[k]=128;//Binarizacion
        if(Imagen1->imx[k]>umbral2)
            Imagen2->imx[k]=255;
    }
    
    gcPutImgBmp("multinivel.bmp",Imagen2);
