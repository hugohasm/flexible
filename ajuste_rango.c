...
gcIMG* Binarizar(gcIMG *img);
gcIMG* AjusteDeRango(gcIMG *img);
gcIMG* AjusteLogaritmico(gcIMG *img);

int main(void)
{ //Declarar un Puntero a imagen
    gcIMG *Imagen1,*Imagen2,*Imagen3,*Imagen4;
    unsigned int k;
    Imagen1=gcGetImgBmp("uno.bmp");
    Imagen2=gcGetImgBmp("dos.bmp");
    Imagen3=gcNewImg(640,480); //Generando nueva imagen
    //Sumando imagenes 
    /*for(k=0;k<Imagen1->size;k++)
        Imagen3->imx[k]=Imagen1->imx[k]+Imagen2->imx[k];*/
    //Restando imagenes
    for(k=0;k<Imagen1->size;k++)
        Imagen3->imx[k]=Imagen1->imx[k]-Imagen2->imx[k];

    //Probando funcion de ajuste de rango
    Imagen4=AjusteDeRango(Imagen3);
    gcPutImgBmp("rango.bmp",Imagen4);
...
gcIMG* Binarizar(gcIMG *img)
{
    gcIMG *ImgR;
    unsigned int k;
    ImgR=gcNewImg(img->ancho,img->alto);
...
gcIMG* AjusteDeRango(gcIMG *img)
{
    ...
    //Buscar valores maximo y minimo de la matriz
    for(k=1;k<img->size;k++)
    {
        if(img->imx[k]<min)
            min=img->imx[k];
        if(img->imx[k]>max)
    ...
    //printf("minimo: %i \n maximo: %i\n",min,max);
//Aplicando formula a valor de cada pixel
//PixelAjustado = ( (Pixel-minimo)/(maximo-minimo) )*255
    for(k=0;k<img->size;k++)
        ImgR->imx[k]=255.0*((img->imx[k]-min)/(max-min));//castear para ser tipo byte
