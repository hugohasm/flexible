...
gcIMG* SuavizadoPromedio(gcIMG *img);
void Convolucion(void);

int main(void)
{ //Declarar un Puntero a imagen
  /*gcIMG *Img1,*Img2;
  Img1=gcGetImgBmp("dos.bmp"); //Para abrir un archivo de imagen
  Img2=SuavizadoPromedio(Img1);
  gcPutImgBmp("Suavizado.bmp",Img2);
...
void Convolucion()
{
  float imagen[5][5]={17,34,1,8,15,
                      23,5,7,14,16,
                      4,6,13,20,22,
                      10,12,19,21,3,
                      11,18,25,2,9};
  /*float H[3][3]={ 8,1,6,   //Mascara, nucleo o kernel de convolucion
                  3,5,7,
                  4,9,2 };*/
  //Rotar la mascara
  float H[3][3]={ 2,9,4,
...
for(i=1;i<4;i++)
      for(j=1;j<4;j++)
      {
        R[i-1][j-1]= imagen[i-1][j-1]*H[0][0]+
                     imagen[i-1][j]*H[0][1]+
                     imagen[i-1][j+1]*H[0][2]+
                     imagen[i][j-1]*H[1][0]+
                     imagen[i][j]*H[1][1]+
                     imagen[i][j+1]*H[1][2]+
                     imagen[i+1][j-1]*H[2][0]+
                     imagen[i+1][j]*H[2][1]+
                     imagen[i+1][j-1]*H[2][2];
     } 
...
/*
General
        R[i-1][j-1]= imagen[i-1][j-1]*H[i-1][j-1]+
                     imagen[i-1][j]*H[i-1][j]+
                     imagen[i-1][j+1]*H[i-1][j+1]+
                     imagen[i][j-1]*H[i][j-1]+
                     imagen[i][j]*H[i][j]+
                     imagen[i][j+1]*H[i][j+1]+
                     imagen[i+1][j-1]*H[i+1][j-1]+
                     imagen[i+1][j]*H[i+1][j]+
  ...
  gcIMG* SuavizadoPromedio(gcIMG *img)
  {
    ...
    for(i=1;i<img->alto-1;i++)
      for(j=1;j<img->ancho-1;j++)
        ImgR->imx[(i-1)*ImgR->ancho+(j-1)]= (img->imx[(i-1)*a+j-1]+img->imx[(i-1)*a+j+0]+img->imx[(i-1)*a+j+1]+
                                            img->imx[(i+0)*a+j-1]+img->imx[(i+0)*a+j+0]+img->imx[(i+0)*a+j+1]+
                                            img->imx[(i+1)*a+j-1]+img->imx[(i+1)*a+j+0]+img->imx[(i+1)*a+j+1])/9;
